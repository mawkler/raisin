...
pub(crate) fn run(
    ...
    let app = gtk4::Application::new(Some("com.github.mawkler.raisin"), Default::default());

    let app_for_activate = app.clone();
    let state_for_activate = state.clone();
    let selected_for_activate = selected_window.clone();

    app.connect_activate(move |_app| {
        let app = app_for_activate.clone();
        let state = state_for_activate.clone();
        let selected = selected_for_activate.clone();
